...
const Header = () => {
  ...
  return (
    <>
      <motion.header 
        className={`fixed top-5 left-0 right-0 mx-auto z-50 transition-all duration-500 border border-gray-100 rounded-full shadow-sm ${
          scrolled 
            ? 'w-[calc(80%-2rem)] max-w-6xl py-2 px-4 bg-white/95 backdrop-blur-md' 
            : 'w-[90%] max-w-7xl py-3 px-5 bg-white'
        }`}
        ...
        <div className="grid grid-cols-3 items-center w-full">
          ...
          <nav className="flex items-center justify-center">
            <div className="hidden md:flex items-center space-x-2">
              ...
              <NavLink href="/suppliers" isActive={pathname === '/suppliers'}>
                Suppliers
              </NavLink>
          ...
          <div className="flex justify-end">
            <div className="hidden md:flex items-center gap-2">
              ...
              <Link href="/login">
                <motion.button 
                  className={`rounded-full border border-[#683cec] text-[#683cec] text-sm font-medium transition-all cursor-pointer relative overflow-hidden ${
                    scrolled ? 'px-3.5 py-1.5 text-xs' : 'px-4 py-2 text-sm'
                  }`}
                  whileHover={{ 
                    scale: 1.03, 
                    boxShadow: '0 0 15px rgba(104, 60, 236, 0.2)' 
                  }}
                  transition={{ duration: 0.2 }}
                >
                  <span className="absolute inset-0 bg-[#683cec] opacity-0 hover:opacity-10 transition-opacity duration-300"></span>
        ...
        {mobileMenuOpen && (
          ...
          >
            <nav className="flex flex-col space-y-6 mt-8">
              ...
              <MobileNavLink href="/suppliers" onClick={() => setMobileMenuOpen(false)} isActive={pathname === '/suppliers'}>
                Suppliers
              </MobileNavLink>
            ...
            <div className="mt-8 flex flex-col space-y-4">
              ...
              <Link href="/contact" onClick={() => setMobileMenuOpen(false)}>
                <motion.button 
                  className="w-full py-2.5 rounded-full bg-[#2B2F56] text-white text-center font-medium text-sm cursor-pointer relative overflow-hidden mb-3"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  transition={{ duration: 0.2 }}
                >
              ...
              <Link href="/login" onClick={() => setMobileMenuOpen(false)}>
                <motion.button 
                  className="w-full py-2.5 rounded-full border border-[#683cec] text-[#683cec] text-center font-medium text-sm cursor-pointer relative overflow-hidden"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  transition={{ duration: 0.2 }}
                >
